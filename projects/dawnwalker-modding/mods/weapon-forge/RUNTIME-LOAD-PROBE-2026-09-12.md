...
The probe started and then recorded:
...
The issue is not only the menu's initial object scan. The running game also
cannot resolve the new package by its exact `/Game` path after a direct load
request. Do not treat this packed clone as a usable inventory item.
